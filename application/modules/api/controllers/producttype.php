...
<<<<<<< Updated upstream
	...
	function checkProductType()
	{
		...
		$this->productType   = ( $this->uri->segment(6) ) ? $this->uri->segment(6) : '';
		...
		if ( $is_valid_auth['rc'] == 0 ){
			...
			if ( $this->productType != '' ){
				...
				$response = $this->product_model->checkProductType(urldecode($this->productType));
			}
			else {
				$response['rc']			= 999;
				$response['success']	= false;
				$response['message'][]	= 'required: product type name';
			}
		...
		$log_data = array(
			'log_client_id' => $this->authKey,
			'log_method' 	=> 'checkProductType',
			'log_url' 		=> $this->uri->uri_string(),
			'log_request' 	=> json_encode($this->input->get()),
			'log_response' 	=> json_encode($response),
		);
		$this->apilog_model->apiLog($log_data); //db logs
		$this->api_functions->apiLog(json_encode($log_data),'checkProductType'); //text logs
